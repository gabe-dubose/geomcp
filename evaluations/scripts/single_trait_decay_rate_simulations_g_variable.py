...
def evolve(n_traits, z_0, r, opt_C, opt_A, mu, sigma_m, population_size, sigma_s, expression_regime, generations):
    ...
    # get f_C and f_A for first generation
    if expression_regime[0] == 'f_A':
        f_A = 1
        f_C = 0
    elif expression_regime[0] == 'f_C':
        f_A = 0
        f_C = 1
    ...
    for generation in range(generations): 
        ...
        # define expression 
        if expression_regime[generation] == 'f_A':
            f_A = 1
            f_C = 0
        elif expression_regime[generation] == 'f_C':
            f_A = 0
            f_C = 1
...
# define expression regimes 
expression_regimes = {
    n : ['f_C' if (i + 1) % n == 0 else 'f_A' for i in range(generations)]
...
def fitness_decay(n_runs, r, expression_regime):
    ...
    for run in range(n_runs):
        if run == 0:
            history = evolve(n_traits, z_0, r, opt_C, opt_A, mu, sigma_m, population_size, sigma_s, expression_regime, generations)
...
def decay_rate_space(r_values, expression_regimes, n_runs):
    regime_ids = list(expression_regimes.keys())
    lambda_grid = pd.DataFrame(index=np.round(r_values, 2), columns=regime_ids)

    for r in r_values:
        for n in regime_ids:
            try:
                output = fitness_decay(n_runs, r, expression_regimes[n])
                lambda_val = output["lambda_fit"]
            except Exception as e:
                print(f"Error for r={r}, regime every_{n}: {e}")
                lambda_val = np.nan
            lambda_grid.loc[round(r, 2), n] = lambda_val
        
        print(f"Completed analysis of r = {r}")
        
    return lambda_grid.astype(float)

n_runs = 1
decay_rates = decay_rate_space(r_values, expression_regimes, n_runs)
decay_rates.to_csv('decay_rates_data_temporal.csv')
